...
type tofloatTag struct {
	intfs.TagBase
	elementResolver intfs.ElementResolver
}

func (tag *tofloatTag) init(tagResolver intfs.TagResolver) {
	tag.Name = "to-float"
	tag.TagResolver = tagResolver
	tag.elementResolver = tagResolver.GetElementResolver()
}

func (tag *tofloatTag) Process(tagTokens []interface{}, bindingDataList *list.List) (interface{}, error) {
	...
	if s, ok := resolvedData.(string); ok {
	    ...
	    if err != nil {
	        ...
	        errk.Message = fmt.Sprintf("Tag \"%s\" invalid string \"%s\"", tag.Name, resolvedData)
            return nil, errk
	    }
	    return f, nil
	} else {
